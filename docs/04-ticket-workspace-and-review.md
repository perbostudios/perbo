...
Built: Create, its picker, the Spec pane, which holds the spec, the Explorer pane, the Graph pane, whose inspector reads an epic's criteria and leaves changing them to the chat, the Impact pane, the contract as planning's last tab, where a basic ticket's criteria are edited by hand and its **Confirm contract** reads the plan against the spec only where the spec or the criteria moved since the last reading, refused while problems are open, the tabs each shape of plan is offered ([D-NEW-basic-and-epic-flows](11-open-decisions.md)), and `perbo interview` on Claude and on Codex, the last writing the spec folder, `CONTEXT.md` and ADRs, changing the plan only through the validated edit path, and docked in planning mode as a chat beside every pane but Problems and the contract ([D-101](11-open-decisions.md), [D-102](11-open-decisions.md)).
