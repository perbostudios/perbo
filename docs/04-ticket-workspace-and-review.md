...
`TICKET_TRANSITIONS` in `packages/contracts/src/ticket-transitions.ts` is the whole lifecycle: a move with no row is refused, and every move appends a row to the ticket's `history`. Admission creates a ticket at `plan_review`; `perbo approve` moves it to `ready`. `perbo run --ticket` reads the ticket's spec before anything else, and a stale one takes the ticket to `plan_invalid` rather than starting it ([D-103](11-open-decisions.md)); otherwise it moves the ticket to `provisioning` before the attempt starts, so a run that never returns leaves the ticket saying so, and afterwards walks it through the states the run's result proves (`executing` and `verifying` where an attempt ran, `independent_review` where a review ran) to `pr_open`, `changes_requested` or `failed`; a run that a person's answers took to delivery without a round goes from `provisioning` to `pr_open` on its own row ([D-NEW-a-person-s-answer-closes-a-routed-finding](11-open-decisions.md)). A walk that cannot reach the state its evidence names is refused, never filled in. The desktop's sample host moves its tickets along the same rows, taken from `@perbo/contracts/browser`, and a stop writes it no row, as it writes the host none. `sync` moves `pr_open` to `merged`. A ticket closes at merge or at `closed` ([D-011](11-open-decisions.md)). Nothing puts a ticket in `DRAFT`, `SPECIFYING`, `DONE`, `DEPLOYED` or `OBSERVING`. Side states:
...
- **Drafted** ([D-072](11-open-decisions.md)): `--from owner/repo#N` fetches the issue; `--from-file <file>` reads a Markdown file whose first line is the title; `--from-spec <file>` reads a spec's `spec.md` ([D-103](11-open-decisions.md)). A model drafts the ticket's name ([D-127](11-open-decisions.md)), the outcome, as many criteria as the work has (`test`, `artifact`, `query` or `metric`; `manual` is a person's to state), a proposed scope of as many globs as the work lands in, a rationale, `depends_on`, and — where it divides the work — the nodes of an execution graph with the order it suggests between them, in one closed schema call through the reviewer's transport. The issue or the spec is external-trust data; anything in it addressed to the drafter is found by reading the text and flagged on the draft ([D-035](11-open-decisions.md)). The drafter is shown the board (every ticket in flight whose contract can be read, with its key, state, priority, outcome and scope; one that cannot be read is named on stderr and left off) and the name of every other ticket in the store, whatever its state ([D-127](11-open-decisions.md)), may propose `depends_on` only among those keys, and may open up to eight small files through the reviewer's bounded reader, each recorded on the draft. `--outcome`, `--criterion` and `--path` override the draft's parts; `--criterion` or `--path` also drops the drafted graph, because each replaces what the graph divided, and the graph is then built with `perbo edit --graph-edit`. `--keep-title`, with `--from-spec`, names the ticket after the spec's title as it stands, or its outcome where the spec has no title line, and leaves the spec as it is, for a spec the person titled; the desktop passes it where they did ([D-127](11-open-decisions.md)).
...
- **A pane's save never overwrites what it did not read** (SCP-321). The Spec pane, the Architect and the Impact pane turning a warning into a No-Go each write `spec.md`, and none of them tells the others — but only the two panes' saves go through a check: each says what its writer last read, and the file is compared against that section by section at the moment of the write — never against a clock or a revision the app kept, because two processes write this file and only the bytes are an honest answer. A section its writer did not change takes whatever the file now says; a section only its writer changed takes the writer's text; a section both changed is refused. A refusal writes nothing at all, not even the sections that would have gone through, and carries the file back with it, so the Spec pane shows both texts and the person keeps whichever words they want. Nothing else in the spec is written until they do, because a write is of the whole file and would carry the refused section with it. The Architect writes through its own tools, not this check: what it does with a section a person changed since the Architect last read it is the Architect's own behaviour, not something either pane's save governs.
...
`perbo edit KEY --graph-edit '<json>'` applies one graph edit to a plan, and `perbo edit KEY --undo <n>` reverts one. The edit is one of `add_node`, `split_node`, `merge_nodes`, `delete_node`, `set_criterion`, `set_node_paths`, `add_edge` and `remove_edge`; it is applied to a copy, the result is validated as a contract, an approach and an acyclic graph, and a refused edit changes nothing and says why. Each applied edit is recorded in the draft snapshot with its author (`--author you|interview`, default `you`), a summary, the entity keys it touched (`node:<id>`, `criterion:<id>`, `edge:<from>-><to>`) and each key's value either side. `edit_count` counts only the person's edits, so the Architect's do not raise it. An undo is refused, naming the later edit by number and summary, when a later edit still in force touched any of the same keys ([D-100](11-open-decisions.md)); an undone edit stays in the log, marked undone. `add_edge` and `remove_edge` are the two that still apply after approval, because the order between nodes is approach. An undo the Architect makes is held to the rule its edits are ([D-128](11-open-decisions.md)). The graph changes through no other path: the editor refuses a change to `nodes`, measured against the counter-seal rather than the file a refused edit was left as; `--criterion` is refused on a plan with a graph; and `--path` keeps the graph and is refused where it would leave a node's paths outside the scope. `delete_node` on the last node, with nothing moved or deleted, makes the plan flat again with every criterion kept, and the approach record goes with the graph unless the spec's No-Gos keep it. A contract file carrying nodes that its draft snapshot does not vouch for is refused by every edit, including one that changes nothing.

Built: Create, its picker, the Spec pane, which holds the spec, the Explorer pane, the Graph pane, the Impact pane, the contract as a tab of planning, with Problems below it while a problem is open, the tabs each shape of plan is offered ([D-NEW-basic-and-epic-flows](11-open-decisions.md)), and `perbo interview` on Claude and on Codex, the last writing the spec folder, `CONTEXT.md` and ADRs, changing the plan only through the validated edit path, and docked in planning mode as a chat beside every pane but Problems and the contract ([D-101](11-open-decisions.md), [D-102](11-open-decisions.md)).
...
- `outcome`: one sentence, what will be true afterwards. The branch and the pull request are named from it. The ticket's name is not, and is chosen per [D-127](11-open-decisions.md).
- `acceptance_criteria`: what must be proven, never where the proof will live, because at approval the test usually does not exist. `expected_verification.kind` is `test`, `query`, `metric`, `artifact` or `manual`; `manual` names its reviewer and why it cannot be automated. `requirement_id` is optional and records the spec requirement the criterion was drafted from, `R1` upward ([D-103](11-open-decisions.md)); a plan drafted from an issue carries none. An id the spec does not carry is refused at admission and at approval, where the contract is frozen, and not checked between the two. Where plan and spec disagree on ids is listed on the contract page, and where their words have parted is read by `perbo drift`: on the page before it for an epic, and for a basic ticket as its contract is confirmed, which is refused while a difference is open ([D-128](11-open-decisions.md), [D-NEW-basic-and-epic-flows](11-open-decisions.md)).
...
A person answers each open finding the review routed to them, one that blocks or escalates, in one of three ways ([D-NEW-a-person-s-answer-closes-a-routed-finding](11-open-decisions.md)): their own approach, the approach left to the executor, or the change shipped as it is for that finding. `perbo verdict <KEY> --decide <finding key> --choice approach|let-it-decide|ship-as-is --note "<words>"` records the answer in `.perbo/verdicts.json`; `--note` is required for an approach, and the other two carry their own words where none are given. An answer has its own slot beside the finding's judgement: it never replaces an `endorse` or an `override`, and neither of those replaces it. The AI stand-in cannot record one, and a `security.*` or `context.*` finding can only be shipped as it is. Only a review that judged the whole change and stopped for a person, `changes_requested` or `escalate`, takes an answer: on an `incomplete` or `error` review `--decide` refuses and says why, and a run goes on as though nothing were answered. The desktop's decision screen records one answer per question that takes the three, beside the principle; a finding routed to a person on a review that takes no answer, and any other finding whose closer the reviewer named as a person, is asked for a principle alone. `perbo run` hands the ticket's standing answers to the loop. An answer settles a finding of the review it was taken after, and only of the review it names where the person named one by its id. On the next run, with the branch still at the commit that review judged, a finding handed to the executor joins a remediation round. The person's words reach the brief as data, and the closure verifier checks the round rather than a fresh review ([D-061](11-open-decisions.md)). A finding shipped as it is is closed by the answer. When every standing finding is shipped as it is or verified closed, the run executes nothing and reviews nothing. It goes where an approval goes: the ticket moves from `provisioning` to `pr_open` on the one row that run writes. The review on record is never written again: `perbo inspect` prints each answer under its finding, and the desktop and any later run read the answers and the closure verifications beside the review, so nothing shipped as it is or verified closed is asked or handed again. A round that stalls or exhausts its attempts with a finding handed to the executor still open ends the run `escalated`, for the person to decide again; a round that regressed a check or the scope ends the run as a change request, as any round does. The loop records each answer on its finding in the review it returns, a finding handed to the executor only where the round's verification closed it. With publishing on and no attempt on record that sealed the judged commit, a delivery without a round stops `terminated` and names the cause. On the desktop a decision that answers a finding publishes as the run that stopped for it was going to, and a principle alone publishes nothing. A published pull request is opened under the run that sealed the judged commit and lists the answers under "Decided by a person", by name and never by address, with the words redacted and unable to write a stop marker. A continued remediation whose review routed a finding to a person ends `escalated` while that finding has no answer. The reviewer is not given the answers, so a branch that has moved is reviewed afresh and a finding it raises again is asked again.
...
With `--publish` (or `"publish": true` in `.perbo/config.json`), an `approved` or `escalated` run pushes its branch and opens a pull request against `base_ref`, titled `<KEY>: <outcome>`; the runner does both, holding the credential. Without it nothing is pushed, and an approved ticket still moves to `pr_open` with its branch kept on the machine. A person publishes that branch later through this same delivery, with `perbo run --ticket <KEY> --publish-retained` or the desktop's Merge on GitHub, running nothing again, and what that refuses is [D-NEW-publish-a-retained-branch-later](11-open-decisions.md)'s. The body carries:
...
`perbo interview` is the other session of the person's own, and reaches no endpoint: it runs through the Claude Agent SDK or `codex app-server`, with its own tools in-process and its own rules deciding every call behind either, and writes the spec ([D-102](11-open-decisions.md), [docs/08](08-security-autonomy-and-data.md)). It holds four tools and no more: `edit_plan` and `undo_edit` change the plan through the same validated path `perbo edit` uses and record the Architect as their author, `read_plan` reads it back, and `ask_options` puts questions to the person with the answers to pick from. None of them takes a ticket key: the plan is the one this spec was drafted into. Drafting that plan from the spec is the person's own press, not a tool of the Architect's, and it admits, approves, publishes and merges nothing, by the same construction as the table above.
...
No record depends on one provider. The executor runs through an adapter (`agent_provider`: `claude-cli` or `codex-cli`) on the person's own login, with an API key optional ([D-093](11-open-decisions.md)); the reviewer and the verifier run through `reviewer_provider`. Each role's `effort` or `reviewer_effort` is a level from its provider's table in `@perbo/contracts` (`EFFORT_LEVELS`), or null for the transport's own default. Every record names what ran (adapter, binary and its hash, model, prompt version), so a model or provider changes without changing any record's shape, and a kill switch turns one off. A dollar figure the provider does not report and no rate card covers stays unknown ([D-070](11-open-decisions.md)).
