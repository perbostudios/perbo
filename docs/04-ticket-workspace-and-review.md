...
- **Drafted** ([D-072](11-open-decisions.md)): `--from owner/repo#N` fetches the issue; `--from-file <file>` reads a Markdown file whose first line is the title; `--from-spec <file>` reads a spec's `spec.md` ([D-103](11-open-decisions.md)). A model drafts the ticket's name ([D-NEW-a-ticket-is-named-apart-from-its-board](11-open-decisions.md)), the outcome, as many criteria as the work has (`test`, `artifact`, `query` or `metric`; `manual` is a person's to state), a proposed scope of one to eight globs, a rationale, `depends_on`, and — where it divides the work — the nodes of an execution graph with the order it suggests between them, in one closed schema call through the reviewer's transport. The issue or the spec is external-trust data; anything in it addressed to the drafter is found by reading the text and flagged on the draft ([D-035](11-open-decisions.md)). The drafter is shown the board (every ticket in flight whose contract can be read, with its key, state, priority, outcome and scope; one that cannot be read is named on stderr and left off) and the name of every other ticket in the store, whatever its state ([D-NEW-a-ticket-is-named-apart-from-its-board](11-open-decisions.md)), may propose `depends_on` only among those keys, and may open up to eight small files through the reviewer's bounded reader, each recorded on the draft. `--outcome`, `--criterion` and `--path` override the draft's parts; `--criterion` or `--path` also drops the drafted graph, because each replaces what the graph divided, and the graph is then built with `perbo edit --graph-edit`.
