...
- a write whose destination resolves outside the worktree (`write_outside_worktree`): a redirect, the target of a writer such as `cp`, `mv`, `rm`, `tee`, `dd of=`, `sed -i` or `tar`, a file tool's path, or in inline interpreter code the literal target of a write call (`writeFileSync`, `open(…, 'w')`, `os.remove`, `shutil.rmtree`) or of a redirect or writer in a command the code spawns. A URL, a spawn's `cwd` or `shell`, and a path the code only reads are not writes. The resolver follows quotes, `cd`, `$TMPDIR` (the worktree's `.perbo-tmp/`) and symlinks, and refuses a destination it cannot place;
...
- a program named at run time (a substitution, a variable, `eval`), and inline interpreter code (`node -e`, `python3 -c`, a heredoc or pipe into an interpreter) unless every statement in it is a read-only shape, which includes reading a file anywhere, or a plain write to a literal path inside the worktree;
