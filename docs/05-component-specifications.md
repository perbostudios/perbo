...
Built: Create, its picker, the Spec pane, which writes the spec folder, the Explorer pane, the Graph pane, the Impact pane, the Problems pane every way from the plan to the contract goes through, over `perbo drift` ([D-NEW-the-plan-answers-the-spec-and-says-so](11-open-decisions.md)), `perbo interview` itself on both transports and the chat docking it beside every pane but Problems, and the loop committing the spec folder first on the ticket's branch ([D-101](11-open-decisions.md), [D-102](11-open-decisions.md), [D-103](11-open-decisions.md)). Decided, not built: the precondition `merge: loop` needs before use (SCP-229, [D-041](11-open-decisions.md)).
