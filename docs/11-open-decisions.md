...
- Built: the guard's refusal, with the review's finding behind it, and a repository's standing prohibited list under `paths_prohibited` in `.perbo/config.json`, each entry carrying what put it there. The planning mode explorer writes it, admission folds it into every new ticket's `paths_prohibited`, the guard reads it again when a run starts, so an entry added after admission binds the ticket's later runs, and `perbo doctor` reports it in its own `PROHIBITED` block. Under Windows semantics the guard also refuses a spelling Windows resolves to a prohibited path — another letter case, a trailing dot or space, a stream, a short 8.3 name — which the review's exact match does not see. It refuses a write to a whole directory a prohibited glob covers or names a place inside, too — `rm -r src/generated` under `src/generated/**`, `rm -r src` under the same glob, and the worktree root under any prohibited glob — and to a directory on disk a prohibited glob can match anything inside through a wildcard, inside the allowed paths as much as outside them — `rm -r src/keys` under `**/*.pem` — which the review sees only as the files under it. A `cp`, `mv`, `install` or `ln` into a directory on disk is judged by the path each source takes there, `src/other/a.ts` for `cp src/a.ts src/other/`, and a source that is not a file on disk as a directory under that name. Each source of an `mv` is judged as a write to its own path, since the move removes it from there. A backup a `cp`, `mv`, `ln`, `install`, `rsync` or `sed -i` keeps beside what it replaces, `<dest><suffix>`, is judged as a write too, and a writer's long option is read by any unambiguous prefix, as GNU reads it, so `cp --t=/tmp` is refused as `cp --target-directory=/tmp` is. A `sed` script is read as GNU and BSD `sed` read it: the file a `w` or `W` command or an `s///w` flag names is judged as a write, and its `e` command, its `s///e` flag and a script the line does not spell are refused as a program run. A word a substitution builds where a command whose options can write or run a program still reads options is refused, since it can be the option that writes: `git diff $(git merge-base HEAD main)` is refused and `git diff --end-of-options $(git merge-base HEAD main)` admitted (docs/08).
