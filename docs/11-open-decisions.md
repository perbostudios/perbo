...
### D-NEW-a-ticket-is-named-apart-from-its-board — A ticket's name is the fewest words that tell it apart
...
### D-NEW-the-plan-answers-the-spec-and-says-so — A plan may be rearranged freely and may not quietly promise something else
...
- Built: the chat's own guard, which refuses an `edit_plan` changing the outcome or a
  ...
  from Home or the picker lands there (D-NEW-a-planning-reopens-where-it-was-left); it puts one problem at a time — the first still open, as a card
...
### D-NEW-a-spec-outlives-its-planning — Before the loop is the picker's; the loop is Home's
...
- Built: the host's reading of the spec folder, skipping an entry it cannot parse rather than
  ...
  **Continue the task** carries; `removeSpecFolder` and
  ...
  slug to one folder name, because an admission record is a file in the repository and this ends
  in a recursive delete; the same reading in the sample host, so it behaves as
  the host does; `untouchedPlanning` in `contract-editing.ts`, which is what holds nothing means;
...
### D-NEW-a-planning-reopens-where-it-was-left — Coming back to a planning is coming back to the pane
...
- Decision: a planning reopens on the pane the person was last on in it, whether they left it for
  ...
  open, those land on the Problems pane (D-NEW-the-plan-answers-the-spec-and-says-so). The
  ...
  (D-NEW-a-spec-outlives-its-planning). A planning's contract is a place it is left at too: a
...
### D-NEW-a-planning-starts-with-what-to-build — A planning starts from its repository's question
...
- Decision: a repository picked in Create's picker opens that repository's own page, not a
  ...
  (D-NEW-a-planning-reopens-where-it-was-left), and a planning opened over a spec or a ticket never
  lands on it.
...
- Built: the `ask` route of the renderer (`#ask/<repoId>`), a page of its own beside the planning
  ...
  repository when that is disconnected, and absent from a stored profile as none kept; the picker selecting the repository's row when opened from its
