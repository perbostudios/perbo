...
| `node --test scripts/lint-boundaries.test.mjs` | A package imported past its interface, a module reached past its surface, `export *` in a source file, production code importing test code, a value taken from a package's root where a browser bundles the file, an import across the desktop's host and renderer layers, a `git` or `gh` process outside the module that runs them |
