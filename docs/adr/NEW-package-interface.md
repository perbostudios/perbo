...
`eslint.config.mjs` holds the import rules, and `scripts/lint-boundaries.test.mjs` shows each rule firing on the form it forbids and staying silent on the form it allows. Two migrations are still in flight, each held by a list a file may only leave: the entry files that still use `export *`, in `eslint.config.mjs`, and the packages that still carry their own compiler options instead of the presets, in `scripts/tsconfig-split.test.mjs`. The per-file subpaths two packages still publish go with the callers that import them.
