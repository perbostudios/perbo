...
- **Approval is not a field of a draft.** The input a drafting caller passes has no `approve`, so no combination of text can approve what it drafts. The endpoint and the queue's drafting tick both pass that type; the interview drafts nothing, because drafting the plan from a spec is the person's own press ([D-102](../11-open-decisions.md)).
