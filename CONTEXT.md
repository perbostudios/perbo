...
**Drift**:
Where a plan's promise and its spec's words have parted after a hand edit, or a chat turn that wrote the spec and left the plan; read by a model as the plan is confirmed, where either moved since the last reading, and each difference put to the person as a problem, which holds the confirm until it is resolved ([D-128](docs/11-open-decisions.md)).
